...
window.HyperSpace.prototype = {
    ...
    show: function() {
        ...
        let zoom = new PIXI.filters.ZoomBlurFilter(2,{
            x: game.halfScreenX,
            y: game.halfScreenY
        },0);
...
function StarMash_2() {
    ...
    section.addButton("Generate New Background", {
        click: function() {
    ...
    section.addValuesField("asteroidLayers", "Visible layers", {
        0: 0,
        1: 1,
        2: 2,
        3: 3
...
StarMash_2.prototype.start = function() {
    ...
    let TextureInfo = {
        ImperialShips: {
            ...
            scale: 1,
            resultScale: 0.5,
            // scaling applied to the resulting texture
            useMask: true,
            maskScale: 2,
            // how much scaling the mask needs
            items: {
                ISD_01: [0, 0, 1700, 583],
    ...
    function randomizeTileMasks() {
        ...
        let masks = [map.children[1], map.children[3], map.children[5]];
        ...
        function RandomPosition() {
            ...
            for (let i = 0; i < 3; i++) {
                map.children[i * 2 + 1].position.set(Tools.randInt(-maxX, 0), Tools.randInt(-maxY, 0));
            }
        ...
        Shuffle();
        RandomPosition();
        // this is ignored if using movable mask
    ...
    function LoadTextures() {
        for (let file in TextureInfo) {
            ...
            for (let t in info.items) {
                ...
                let rawTexture = new PIXI.Texture(full_texture,new PIXI.Rectangle(item[0] * info.scale,item[1] * info.scale,item[2] * info.scale,item[3] * info.scale));
                ...
                if (info.useMask) {
                    ...
                    var maskTexture = new PIXI.Texture(full_mask,new PIXI.Rectangle(item[0] * info.scale / maskScale,item[1] * info.scale / maskScale,item[2] * info.scale / maskScale,item[3] * info.scale / maskScale));
    ...
    function createShips() {
        function createFleet(options) {
            ...
            a += angleDelta) {
                ...
                let pt = getCirclePoint(x, y, radius, a);
                // random center
                x = pt.x;
                y = pt.y;
                ...
                let sprite = createSpriteUpdatable(texture, {
                    //distanceFactor: [8, 6],
                    //distanceFactor: [8 * distanceModifier, 8 * distanceModifier],
                    distanceFactor: [options.baseDistanceFactor * distanceModifier, options.baseDistanceFactor * distanceModifier],
    ...
    function crateTileLayer(img, name, scale) {
        var width = Graphics.renderer.width
          , // + config.overdraw,
        height = Graphics.renderer.height;
        // + config.overdraw;
    ...
    function createPlanet(planetIndex=-1) {
        ...
        function PlanetLoaded() {
            ...
            planet.basePosition = [// this numbers where obtained by just testing different values
            // for the extremes and seeing how it looked
            Tools.randInt(-25000, 70000), //Tools.randInt(-20000, 35000)
    ...
    SWAM.resizeLayers = function(width, height) {
        ...
        if (SWAM.planet) {//SWAM.planet.scale.set(game.scale * SWAM.planet.scaleModifier * config.scalingFactor / 5000, game.scale * SWAM.planet.scaleModifier* config.scalingFactor / 5000);
        }
    ...
    function updatePlanet(cx, cy) {
        ...
        let shsX = game.halfScreenX / game.scale;
        // scaled half screen
...
StarMash_2.prototype.injectTextures = function(files, textureInfo, flagTextureInfo, spriteInfo, textures) {
    ...
    var myFiles = {
        ...
        asteroids3: getFilePath("asteroids/asteroids3.png")//xwing: getFilePath("xwing.png"),
...
StarMash_2.mobAdded = function(data, existing, playerId) {
    ...
    if (isProjectile) {
        if (playerId) {
            ...
            if (p.graphicsSet == 0) {//mob.sprites.sprite.tint = 0x88FF88;
            //colorMatrix.hue(120);
            } else {
...
function overridePlayerMethods(player) {
    ...
    player.setupThrusterColor = function() {
        ...
        if (this.sprites.thruster)
            this.sprites.thruster.filters = [satu, colorMatrix];
        if (this.sprites.thruster1)
            this.sprites.thruster1.filters = [satu, colorMatrix];
        if (this.sprites.thruster2)
            this.sprites.thruster2.filters = [satu, colorMatrix];
    }
    ;
    ...
    player.setupGraphics = function(e) {
        ...
        if (typeof window.Glow == 'function')
            Glow(this);
...
SWAM.registerExtension({
    ...
    dependencies: [//"https://cdn.jsdelivr.net/npm/pixi-filters@2.5.0/dist/pixi-filters.js"
    ]
});
